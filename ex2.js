...
app.get('/:category/:subcategory/:id', (req, res) => {
  ...
  const Psubcategory = req.params.subcategory
  const paramsID = Number(req.params.id)

  const findCategory = storeData.find((el) => el.category === Pcategory)
  if (findCategory) {
    const findSubcategory = findCategory.subcategories.find((el) => el.subcategory === Psubcategory)

    if (findSubcategory) {
      const findID = findSubcategory.products.find((el) => el.id === paramsID)
